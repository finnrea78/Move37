...
alembic -c /app/alembic.ini upgrade head || { echo "[wrapper] Migration failed" >&2; kill $PG_PID; exit 1; }

# Seed deterministic data (idempotent)
echo "[wrapper] Seeding deterministic mock data..."
python3 /app/bin/seed.py || { echo "[wrapper] Seeding failed" >&2; kill $PG_PID; exit 1; }
